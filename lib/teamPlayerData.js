...
async function getPlayerInfo(req, player, dataMap, gameData) {
    const playerDetailResponse = await getPlayerData(req, player.id);
    const currentGames = playerDetailResponse.data.history.filter(game => game.round === Number(gameData.currentGameweek));
    const nextGames = playerDetailResponse.data.fixtures.filter(game => game.event === Number(gameData.currentGameweek) + 1);
    let currentGame = {
        team: '',
        score: 0,
        minutes: 0,
    ...
    currentGames.forEach(game => {
        currentGame.team += dataMap.teams[game.opponent_team] + "\n";
        currentGame.score += Number(game.total_points);
        currentGame.minutes += Number(game.minutes);
        currentGame.xGI += Number(game.expected_goal_involvements);
        currentGame.xGC += Number(game.expected_goals_conceded);
        currentGame.xG += Number(game.expected_goals);
        currentGame.xA += Number(game.expected_assists);
        currentGame.ICT += Number(game.ict_index);
    })

    if (currentGames.length > 0) {
        currentGame.xP = player.ep_this;
        if (currentGames.length > 1) {
            currentGame.xP += player.ep_next;
        }
    }
    ...
    currentGame.score += bonusPoints;

    let upcomingGames = [];
    upcomingGames = nextGames.map(game => {
        const oppositionNextTeam = game.is_home ? game.team_a : game.team_h;
        return {
            team: `${dataMap.teams[oppositionNextTeam]} ${game.is_home ? '(H)' : '(A)'}`,
            fdr: game.difficulty,
            xP: 0
        };
    });

    if (currentGames.length < 2 && upcomingGames.length > 0) {
        upcomingGames[0].xP = player.ep_next;
    }
...
const getTeamDetails = async (req, teamID, gameweek, gwLive, fixtureData, fixData, bootstrapData, dataMap) => {
    try {
        ...
        let predictedGWScore = Number(startingPlayers.reduce((total, player) => total + player.expectedPoints, 0));
        ...
        if (captain) {
            ...
            predictedGWScore += Number(captain.expectedPoints);
        }
        ...
        } else if (chipActive === 'BB') {
            benchPlayers.forEach((player) => {
                predictedGWScore += Number(player.expectedPoints);
            });
        }
        predictedGWScore -= transferCost;
        predictedGWScore = predictedGWScore.toFixed(1);
...
const calculatePredictedPoints = async (teamDetails) => {
    if (!teamDetails || !teamDetails.startingPlayers || !teamDetails.benchPlayers) {
        console.error('Failed to get teamdetails to calculate points');
        return 0; // or throw an error, or return a suitable default value
    }
    //TODO: Add predicted points
    const captainValue = teamDetails.chipActive === 'TC' ? 2 : 1;
    let totalPoints = 0;
    teamDetails.startingPlayers.forEach(player => {
        if (player.playStatus === 'played')
            totalPoints += player.expectedPoints;
    });
    ...
    if (captain.playStatus !== 'unplayed') {
        totalPoints += captain.expectedPoints * captainValue;
    } else if (viceCaptain) {
        totalPoints += viceCaptain.expectedPoints * captainValue;
    }
    ...
    // Add points from bench players who have a substatus of 'In'
    teamDetails.benchPlayers.forEach(player => {
        if (player.subStatus === 'In' && player.playStatus === 'played') {
            totalPoints += player.expectedPoints;
        }
    });
