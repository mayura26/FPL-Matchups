...
const getTeamDetails = async (req, teamID, gameweek, gwLive, fixtureData, bpsData, bootstrapData, dataMap) => {
    try {
        ...
        const activePlayers = startingPlayers.filter(player => player.playStatus === 'playing').length + benchPlayers.filter(player => player.subStatus === 'In' && player.playStatus === 'playing').length;
        const remainPlayer = startingPlayers.filter(player => player.playStatus === 'notplayed').length + benchPlayers.filter(player => player.subStatus === 'In' && player.playStatus === 'notplayed').length;
