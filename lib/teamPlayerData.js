...
// IN-PROGRESS: live and remain not showing right when dgw
// IN-PROGRESS: no symbol if the player vs is benched
...
const getTeamDetails = async (req, teamID, gameweek, gwLive, fixtureData, fixData, bootstrapData, dataMap) => {
    try {
        ...
        return { startingPlayers, benchPlayers, transferCost, activePlayers, remainPlayer, chipActive };
    } catch (error) {
    ...
    async function getPlayerDetails(players, teamResponse, playersInfo, isCurrentGameweek) {
        return await Promise.all(players.map(async (player) => {
            ...
            if (playerInfo.chance_of_playing_this_round != null && playerInfo.chance_of_playing_this_round == 0 && minutesPlayed == 0 && isCurrentGameweek) {
                ...
                } else if ((currentTimeUTC - kickoffTimeUTC >= 5 * 60 * 1000) && minutesPlayed == 0) {
