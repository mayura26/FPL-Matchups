...
function getPlayerBonus(fixData, playerID) {
    let bonusPoints = 0;
    const playerBonus = fixData.data.find(fixture => {
        const fixInfo = fixture.fixInfo;
        if (fixInfo && !fixInfo.bonusAdded && fixInfo.BPSData) {
            const playerFound = fixInfo.BPSData.find(player => player.element === playerID);
