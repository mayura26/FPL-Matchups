...
const calculateBPS = async (req) => {
    ...
    if (cachedData) {
        ...
        if (todaysData && !todaysData.bonus_added) {
            ...
            todaysGames.forEach(game => {
                const gameStarted = game.started && (currentTime - new Date(game.kickoff_time) >= thirtyMinutes);
                const fixInfo = {
                    teamHome: dataMap.teams[game?.team_h] || 0,
                    teamAway: dataMap.teams[game?.team_a] || 0,
                    teamHomeScore: dataMap.teams[game?.team_h_score] || 0,
                    teamAwayScore: dataMap.teams[game?.team_a_score] || 0,
                ...
                game.stats.forEach(stat => {
                    if (stat.identifier === 'bps') {
                        const bps = stat;
                        const bpsPlayers = bps.a.concat(bps.h);
                        bpsPlayers.forEach(player => {
                            player.fixInfo = fixInfo;
                            player.started = gameStarted;
                        });
                    } else {
                        const statInfo = {
                            ...
                            team: dataMap.teams[playersInfo.find(playerI => playerI.id === player.element).team]
                        }
            ...
            // Loop all games and check if started 
            // if started get BPS
            // for all games get fixture data
            // for all games get goal/assist data
            // fixid, fixdetails: [], BPS, goals, Assists

            let bpsPlayers = [];
            todaysGamesStarted.forEach(game => {
            ...
            todaysGamesNotStarted.forEach(game => {
                const blankPlayer = {
                    element: 0,
                    value: 0,
                    fix: game.id,
                ...
                bpsPlayers = [...bpsPlayers, blankPlayer];
            });
